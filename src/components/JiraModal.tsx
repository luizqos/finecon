...
interface JiraModalProps {
  ...
  jiraData: {
    dataSel: string;
    grupos: Array<{ titulo: string; lista: any[] }>;
...
export function JiraModal({ isOpen, onClose, jiraData, metrics, onCopy }: JiraModalProps) {
    ...
    <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="bg-[#212529] text-white p-4 flex justify-between items-center">
          ...
          <button onClick={onClose} className="hover:text-gray-400 text-2xl">&times;</button>
        </div>
          ...
          {jiraData.grupos.map((g, idx) => g.lista.length > 0 && (
            <div key={idx} className="space-y-1">
              ...
              <div className="bg-gray-50 border rounded p-2 max-h-24 overflow-y-auto">
                {g.lista.map((p: any) => (
                  <div key={p.e2e} className="text-[10px] font-mono text-[#d63384] font-bold">
                    {p.e2e}
                  </div>
                ))}
              </div>
        ...
        <div className="p-4 bg-gray-50 border-t flex justify-end">
          <button onClick={onCopy} className="bg-[#0d6efd] text-white px-6 py-2 rounded font-bold hover:bg-blue-700 transition-colors shadow-lg">
            Copiar para o JIRA
          </button>
        </div>
      </div>
